...
NoArvBin BuscarNoArvore(char *arquivo, char *chave, int *index)
{
...
NoArvBin LerNoArvore(char *arquivo, int rrn)
{
    ...
    for (int i = 0; i < no.nroChavesNo; i++)
    {
        ...
        fread(no.C, sizeof(long), 1, arqBin);
        fread(no.PR, sizeof(long), 1, arqBin);

        printf("P: %d | C: %ld | PR: %ld |\n", no.P[i], no.C[i], no.PR[i]);
    }

    // printf("LEITURA NO: %d | %d | %d\n", no.C[0], no.C[1], no.C[2]);
